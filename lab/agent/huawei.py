...
class VRPV8:
    def __init__(self):
        super.__init__('huawei_vrpv8', '192.168.88.201', 'huaweiuser', 'huawei@123')

    def get_config(self):
        cmd = 'dis cur'
        info = self.dev_conn.send_command(cmd)
        config_info = 'aaa'.join(info.split('aaa')[1])
        return config_info
    ...
    def reconver_interface(self, if_name):
        ...
        self.dev_conn.send_command(command_string=cmd_list, expect_string=r']')
    ...
    def post_route(self, dst_n, mask, next_hop):
        ...
        for cmd in cmd_list:
            for cmd in cmd_list:
                self.dev_conn.send_command(command_string=cmd, expect_string=r']')
